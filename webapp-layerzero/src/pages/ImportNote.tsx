...
import { formatUSDC, CHAIN_CONFIGS } from '../lib/chains'
...
function ImportNote() {
  ...
  const handleImport = () => {
    ...
    try {
      // Set chainId from note data or current chain
      const noteWithChain = {
        ...parsedNote,
        chainId: parsedNote.chainId || chainId,
      }
      addNote(noteWithChain)
      setSuccess(true)
      setNoteJson('')
      setParsedNote(null)
    } catch (err) {
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-white font-medium">
                  {formatUSDC(BigInt(parsedNote.balance), parsedNote.chainId || chainId)} USDC
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Chain:</span>
                <span className="text-slate-300">
                  {CHAIN_CONFIGS[parsedNote.chainId || chainId]?.name || `Chain ${parsedNote.chainId || chainId}`}
                </span>
              </div>
              <div className="flex justify-between">
