...
// BN254 field size (snark scalar field)
...
/**
 * Derive note parameters (randomness, nullifierSecret) from ECDH shared secret.
 * Both sender and recipient can independently derive the same values.
 *
 * Sender: ECDH(ephemeralPrivKey, recipientViewingPubKey)
 * Recipient: ECDH(viewingPrivKey, ephemeralPubKey)
 */
export function deriveNoteParams(sharedSecret: Uint8Array): {
  randomness: bigint
  nullifierSecret: bigint
} {
  const secretHex = bytesToHex(sharedSecret)
  const randomnessHash = keccak256((`${secretHex}:randomness`) as `0x${string}`)
  const nullifierHash = keccak256((`${secretHex}:nullifier`) as `0x${string}`)
  return {
    randomness: BigInt(randomnessHash) % FIELD_SIZE,
    nullifierSecret: BigInt(nullifierHash) % FIELD_SIZE,
  }
}

/**
 * Generate stealth transfer data including ECDH-derived note parameters.
 * Returns everything needed for the sender to create the transfer,
 * and the ephemeral public key that lets the recipient derive the same note params.
 */
export function generateStealthTransfer(
  recipientSpendingPubKey: Uint8Array,
  recipientViewingPubKey: Uint8Array,
): {
  stealthData: StealthDataForContract
  sharedSecret: Uint8Array
  noteParams: { randomness: bigint; nullifierSecret: bigint }
} {
  const ephemeralPrivateKey = secp256k1.utils.randomPrivateKey()
  const ephemeralPubKey = secp256k1.getPublicKey(ephemeralPrivateKey, false)
  ...
  const noteParams = deriveNoteParams(sharedSecret)

  return {
    stealthData: {
      stealthAddress,
      ephemeralPubKeyX: ephemeralPoint.x,
      ephemeralPubKeyY: ephemeralPoint.y,
...
/**
 * Recipient: try to derive note params from ephemeral pub key coordinates.
 * Returns null if ephemeralPubKey is zero (unregistered sender).
 */
export function tryDeriveNoteFromEphemeralKey(
  viewingPrivateKey: Uint8Array,
  ephemeralPubKeyX: bigint,
  ephemeralPubKeyY: bigint,
): { randomness: bigint; nullifierSecret: bigint } | null {
  if (ephemeralPubKeyX === 0n && ephemeralPubKeyY === 0n) return null

  try {
    const ephemeralPoint = secp256k1.ProjectivePoint.fromAffine({
      x: ephemeralPubKeyX,
      y: ephemeralPubKeyY,
    })
    const ephemeralPubKey = ephemeralPoint.toRawBytes(false)
    const sharedSecret = secp256k1.getSharedSecret(viewingPrivateKey, ephemeralPubKey)
    return deriveNoteParams(sharedSecret)
  } catch {
    return null
  }
}
