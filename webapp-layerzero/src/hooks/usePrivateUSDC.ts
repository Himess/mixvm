...
function getChainConfig(chainId: number | undefined) {
  if (!chainId || !CHAIN_CONFIGS[chainId]) {
    ...
    return {
      contractAddress: CONTRACTS.baseSepolia.bridge,
      usdcAddress: CHAIN_CONFIGS[84532].usdc,
      usdcDecimals: CHAIN_CONFIGS[84532].usdcDecimals,
      deployBlock: CHAIN_CONFIGS[84532].deployBlock,
      rpc: CHAIN_CONFIGS[84532].rpc,
      chainId: 84532,
    }
  }
...
// Helper: get gas params that work on all chains (including Arb Sepolia which lacks eth_maxPriorityFeePerGas)
async function getGasParams(provider: ethers.Provider): Promise<Record<string, unknown>> {
  try {
    ...
    if (feeData.gasPrice) {
      return { gasPrice: (feeData.gasPrice * 150n) / 100n }
    }
  } catch (e) {
    console.warn('getFeeData failed, falling back to eth_gasPrice:', e)
    try {
      // Fallback: use legacy gasPrice RPC call
      const block = await provider.getBlock('latest')
      if (block?.baseFeePerGas) {
        const gasPrice = (block.baseFeePerGas * 200n) / 100n // 2x baseFee as safety margin
        return { gasPrice }
      }
    } catch {
      console.warn('Block baseFee fallback also failed, letting wallet decide gas')
    }
  }
  return {} // Let wallet/MetaMask decide gas pricing
}
...
// ABI for PrivateLZBridge contract interactions
const CONTRACT_ABI = [
  'function deposit(uint256 amount, bytes32 commitment) external',
  'function withdraw(address recipient, uint256 amount, bytes32 nullifier, bytes32 newCommitment, bytes32 merkleRoot, uint256[8] proof) external',
...
export function usePrivateUSDC() {
  ...
  const DEPLOY_BLOCK = chainConfig.deployBlock
  const RPC_URL = chainConfig.rpc
  const IS_NATIVE_USDC = false // LayerZero version uses ERC-20 USDC only
  ...
  // Filter notes by current chain (memoized to prevent infinite re-renders)
  const chainNotes = useMemo(
    () => notes.filter((n) => !n.chainId || n.chainId === chainId),
    [notes, chainId]
  )

  // Calculate balance from chain-filtered notes
  useEffect(() => {
    const total = chainNotes.reduce((sum, note) => sum + BigInt(note.balance), BigInt(0))
    setPrivateBalance(total.toString())
  }, [chainNotes, setPrivateBalance])
  ...
  }, [CONTRACT_ADDRESS, DEPLOY_BLOCK, RPC_URL])
  ...
  const deposit = useCallback(async (amount: bigint) => {
    ...
    try {
      ...
      console.log('4.1. Bridge Contract Address:', CONTRACT_ADDRESS)
      console.log('4.2. Connected Chain ID:', chainId)
      ...
      if (IS_NATIVE_USDC) {
        ...
        const gasParams1 = await getGasParams(signer.provider!)
        tx = await signer.sendTransaction({ to: CONTRACT_ADDRESS, data, value: amount, ...gasParams1 } as Parameters<typeof signer.sendTransaction>[0])
      } else {
        ...
        if (currentAllowance < amount) {
          console.log('12. Approving USDC spend (max allowance)...')
          console.log('12.1. Approving spender:', CONTRACT_ADDRESS)
          const maxApproval = ethers.MaxUint256
          console.log('12.2. Approval amount:', maxApproval.toString())
          const approveTx = await usdcContract.approve(CONTRACT_ADDRESS, maxApproval)
          console.log('13. Approve TX:', approveTx.hash)
          const approveReceipt = await approveTx.wait()
          console.log('14. Approve receipt status:', approveReceipt?.status)
          console.log('14.1. Approve confirmed, block:', approveReceipt?.blockNumber)

          // Check if approve actually succeeded
          if (!approveReceipt || approveReceipt.status !== 1) {
            throw new Error('USDC approve transaction failed - check if you have enough ETH for gas')
          }

          // Double-check allowance after approval (with retry for slow RPC sync)
          let newAllowance = await usdcContract.allowance(signerAddress, CONTRACT_ADDRESS)
          console.log('14.5. New allowance:', newAllowance.toString())

          // RPC might not have synced yet - retry once after 2s delay
          if (newAllowance < amount) {
            console.log('14.5.1. Allowance not synced yet, waiting 2s and retrying...')
            await new Promise(resolve => setTimeout(resolve, 2000))
            newAllowance = await usdcContract.allowance(signerAddress, CONTRACT_ADDRESS)
            console.log('14.5.2. Retry allowance:', newAllowance.toString())
          }

          // If still doesn't match after retry, warn but proceed anyway since TX receipt was successful
          if (newAllowance < amount) {
            console.warn('14.6. WARNING: Allowance read still low after retry, but approve TX succeeded (status 1). Proceeding with deposit...')
          }
        } else {
          console.log('12. Allowance sufficient, skipping approve')
        }

        // Call deposit (PrivateLZBridge)
        console.log('15. Calling deposit...')
        const iface = new ethers.Interface(CONTRACT_ABI)
        const data = iface.encodeFunctionData('deposit', [amount, commitmentBytes32])

        const gasParams2 = await getGasParams(signer.provider!)
        tx = await signer.sendTransaction({ to: CONTRACT_ADDRESS, data, ...gasParams2 } as Parameters<typeof signer.sendTransaction>[0])
      }
  ...
  }, [isConnected, walletClient, getSigner, setLoading, setError, addNote, addTransaction, updateTransaction, refreshMerkleTree, chainId, CONTRACT_ADDRESS, USDC_ADDRESS, USDC_DECIMALS])
  ...
  const withdraw = useCallback(async (amount: bigint, recipient: string) => {
    ...
    // Find a note with sufficient balance (chain-filtered)
    const note = chainNotes.find((n) => BigInt(n.balance) >= amount)
    if (!note) {
    ...
    try {
      ...
      const contractMerkleRoot = await contract.getLastRoot()
      ...
      // Use contract's merkle root as ground truth (workaround for JS tree root mismatch)
      const contractRootBigInt = BigInt(contractMerkleRoot)
      console.log('12.1. Using contract root for circuit input:', contractRootBigInt.toString())
      ...
      // Flatten proof to uint256[8]: [pA[0], pA[1], pB[0][0], pB[0][1], pB[1][0], pB[1][1], pC[0], pC[1]]
      const flatProof: bigint[] = [
        proofData.pA[0], proofData.pA[1],
        proofData.pB[0][0], proofData.pB[0][1],
        proofData.pB[1][0], proofData.pB[1][1],
        proofData.pC[0], proofData.pC[1],
      ]
      ...
      const data = iface.encodeFunctionData('withdraw', [
        ...
        contractMerkleRoot,        // bytes32 merkleRoot (from contract)
        flatProof.map((x: bigint) => x.toString()),
      ])

      const gasParams = await getGasParams(signer.provider!)
      const tx = await signer.sendTransaction({ to: CONTRACT_ADDRESS, data, gasLimit: 3000000, ...gasParams } as Parameters<typeof signer.sendTransaction>[0])
  ...
  }, [isConnected, walletClient, chainNotes, getSigner, setLoading, setError, addNote, removeNote, addTransaction, updateTransaction, CONTRACT_ADDRESS, USDC_DECIMALS, chainId])
  ...
  const transfer = useCallback(async (
    ...
    try {
      ...
      const gasParams3 = await getGasParams(signer.provider!)
      const tx = await signer.sendTransaction({ to: CONTRACT_ADDRESS, data, gasLimit: 3000000, ...gasParams3 } as Parameters<typeof signer.sendTransaction>[0])
  ...
  }, [isConnected, walletClient, chainNotes, getSigner, setLoading, setError, addNote, removeNote, addTransaction, updateTransaction, refreshMerkleTree, CONTRACT_ADDRESS, chainId])
