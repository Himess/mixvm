/**
 * Test Route 6: Ethereum Sepolia -> Base Sepolia
 *
 * Usage:
 *   npx hardhat run scripts/tests/test-route-6-ethereum-to-base.ts --network ethereumSepolia
 */
...
async function main() {
    ...
    console.log("TEST ROUTE 6: Ethereum Sepolia -> Base Sepolia");
    console.log("=".repeat(60));
    ...
    if (ethBalance < ethers.parseEther("0.01")) {
        console.log("ERROR: Insufficient ETH. Use faucet or bridge from Base Sepolia.");
        return;
    }

    const usdcAddress = USDC_ADDRESSES[11155111];
    ...
    const bridgeAddress = BRIDGE_ADDRESSES[11155111];
    if (!bridgeAddress) {
    ...
    console.log("Destination Domain:", CCTP_DOMAINS.baseSepolia, "(Base Sepolia)");
    ...
    console.log("\nNext: Call privateTransferCrossChain with domain=6 (Base)");
    console.log("=".repeat(60));
}
